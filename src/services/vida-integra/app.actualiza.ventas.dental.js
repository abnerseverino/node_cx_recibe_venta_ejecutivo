...
const capturaLooker = async () => {
  try {
    ...
    const lookerURL =
      "https://lookerstudio.google.com/u/0/reporting/3a97ace1-9981-4e08-a99b-7e439a960812/page/p_iuv80bd2ud";
    ...
    while (pageCount <= 3) { 
      ...
      for (const [index, row] of registrosPagina.entries()) {
        try {
          ...
          const plan = row[5];
          const precio = parseFloat(row[6].replace(/[^0-9.-]+/g, ""));
          let estado = row[7];
          ...
          const query1 = `
            ...
            WHERE
              ven_eje_campana_id in (52,54) AND 
          ...
          const existeQuery = `
            ...
            WHERE ven_eje_campana_id in (52,54)
    ...
    const ajusteQuery = `
      ...
      WHERE ven_eje_campana_id in (52,54) AND 
