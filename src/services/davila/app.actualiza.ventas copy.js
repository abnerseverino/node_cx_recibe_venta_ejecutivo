...
const capturaLooker = async () => {
  try {
    ...
    const lookerURL = "https://lookerstudio.google.com/u/0/reporting/3a97ace1-9981-4e08-a99b-7e439a960812/page/sAMLE";
    ...
    while (true) {
      ...
      for (const [index, row] of registrosPagina.entries()) {
        try {
          ...
          console.log(
            `📌Fila ${index}: ID : CAMPANA: ${campanaId}, MES : ${MesVentaID}, Rut: ${rut}, ${id}, Certificado: ${certificado},  Fecha: ${row[3]} --- ${fecha_contrato}, Plan: ${plan}, Precio: ${precio}, Estado: ${estado}, Beneficiarios: ${beneficiarios}`
          );
          ...
          const query = `
            ...
            WHERE
                ven_eje_campana_id = $4
                AND ven_eje_mes_venta_id = $5
                AND ven_eje_rut_cliente= $6
